...
void parseParameters(int argc, char *argv[]) {
	...
	static struct option long_options[] =
			{ { "fct", required_argument, 0, 'f' }, { "caching", required_argument, 0, 'c' }, { "eps", required_argument, 0, 'e' }, { "cb", required_argument, 0, 'b' }, { "runs", required_argument, 0, 't' }, { "maxflips", required_argument, 0, 'm' }, { "printSolution", no_argument, 0, 'a' }, { "help", no_argument, 0, 'h' },{ "assignment", required_argument, 0, 'l' },{ "flips", required_argument, 0, 'd' }, { 0, 0, 0, 0 } };

	while (optind < argc) {
		int index = -1;
		struct option * opt = 0;
		int result = getopt_long(argc, argv, "f:e:c:b:t:m:a:l:dh", long_options, &index); //
		if (result == -1)
			break; /* end of list */
		switch (result) {
		case 'h':
